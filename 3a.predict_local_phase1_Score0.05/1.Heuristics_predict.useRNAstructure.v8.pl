#!/usr/bin/perl
die "perl $0 enriched_pixels cluster_of_pixels.bedpe\n" if(@ARGV != 2);
...
foreach my $id (@running_loops){
	...
	#test block#############################
	my $tmp_prefix="phase1a_cycle$run_cycle";
	visualize_by_IGV($tmp_prefix);
	visualize_by_ct($tmp_prefix);
}

write_constrains("phase1a_final");
visualize_by_dot("phase1a_final");
visualize_by_IGV("phase1a_final");
visualize_by_ct("phase1a_final");
...
sub update_resolved_bases{
	foreach my $left_pair (sort {$a<=>$b} keys %dynamic_basepairs){
		my $right_pair=$dynamic_basepairs{$left_pair};
		if($right_pair <= $left_pair){
			next;
		}
		foreach ($left_pair..$right_pair){
			$resolved_bases{$_}=1;
		}
	}
}
